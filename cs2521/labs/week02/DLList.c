...
void DLListBefore (DLList L, char *it)
{
	// Base Case
	assert (L != NULL);
	// what is the difference between L->first and L->last
	...
	if (L->curr == NULL) {
		L->first = new;
		L->last = new;
	} else if (L->first == L->last && L->curr == L->last) {
		L->curr->prev = new;
		new->next = L->curr;
		new->prev = NULL;
		L->first = new;
		// Do I have to change the curr now
	// Case when it is the last
...
{	
	...
	assert (L != NULL);
	if (L->first == L->last && L->last == L->curr) {
		...
		DLListNode *temp_next = L->curr->next;
		DLListNode *temp_prev = L->curr->prev;
