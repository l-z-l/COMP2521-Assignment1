...
void IntListInsertInOrder(IntList L, int v)
{
	...
	// Creating the temp pointer
	struct IntListNode *temp = L->first;
	// Creating a new node for v
	struct IntListNode *n = newIntListNode(v);
	// (a) empty list
	if (L->first == NULL)
	{
	...
	{
		// Initialing the previous_value
		previous_value = L->first->data;
		struct IntListNode *temp_2 = L->first;
		// (d) second-smallest value
		// (e) second-largest value
		// (f) value somewhere in the middle.
		for (temp = L->first->next; temp != NULL; temp = temp->next)
		{
			if (v >= previous_value && v <= temp->data) 
			{
				n->next = temp;
				temp_2->next = n;
				break;
			...
			temp_2 = temp_2->next;
			previous_value = temp->data;
		}
	}
	// Dont forget to increment to size
	L->size++;
}
